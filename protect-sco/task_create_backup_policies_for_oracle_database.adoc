...
Vedere link:task_view_oracle_databse_backups_and_clones_in_the_topology_page.html["Visualizzare i backup e i cloni dei database Oracle nella pagina topologia"].
...
+
SnapCenter consente di utilizzare le variabili di ambiente predefinite quando si eseguono il prescrittivo e postscript. link:predefined-environment-variables-prescript-postscript-backup.html["Scopri di più"^]
